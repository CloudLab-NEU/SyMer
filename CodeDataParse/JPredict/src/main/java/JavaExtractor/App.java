...
//
//
//    public static void extractDir(CommandLineValues commandLineValues) {
//        ThreadPoolExecutor executor = (ThreadPoolExecutor) Executors.newFixedThreadPool(commandLineValues.NumThreads);
//        LinkedList<ExtractFeaturesTask> tasks = new LinkedList<>();
//
//        try {
//            Files.walk(Paths.get(commandLineValues.Dir)).filter(Files::isRegularFile)
//                    .filter(p -> p.toString().toLowerCase().endsWith(".java")).forEach(f -> {
//                ExtractFeaturesTask task = new ExtractFeaturesTask(commandLineValues, f);
//                tasks.add(task);
//            });
//        } catch (IOException e) {
//            e.printStackTrace();
