...
public class Property {
    ...
    public Property(Node node, boolean isLeaf, boolean isGenericParent) {
        ...
        SimpleName nameToSplit = new SimpleName(node.toString());
        if (isGenericParent) {
            ...
            if (isLeaf) {
                ...
                // 如果它是算作叶子的通用父级
                // 则当它作为父级参与路径时
                // 它应该是GenericClass而不是简单的Class或InterfaceType
                Type = "GenericClass";
            }
        }
        ArrayList<String> splitNameParts = Common.splitToSubtokens(nameToSplit.getIdentifier());
